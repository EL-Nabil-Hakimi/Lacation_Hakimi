          ...
          <div class="col-md-12">
            ...
            <div class="user-info p-4 bg-light rounded" >
              <form id="photoForm" method="post" enctype="multipart/form-data" action="/updateinfouser">
                ...
                @csrf
                <input type="hidden" name="user_id" value="{{ $user[0]->client->id }}">
                <input type="hidden" name="id" value="{{ $user[0]->id }}">
                <div class="mb-3">
                  <label for="cin" class="form-label">CIN</label>
                  <input type="text" id="cin" name="cin" class="form-control" value="{{ $user[0]->client->cin }}" readonly>
                </div>
                
                <div class="mb-3">
                  <label for="nom" class="form-label">Nom</label>
                  <input type="text" id="nom" name="nom" class="form-control" value="{{ $user[0]->client->nom }}" readonly>
                </div>
  
                <div class="mb-3">
                  <label for="prenom" class="form-label">Prénom</label>
                  <input type="text" id="prenom" name="prenom" class="form-control" value="{{ $user[0]->client->prenom }}" readonly>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <input type="tel" id="phone" name="phone" class="form-control" value="{{ $user[0]->client->phone }}"  readonly>
                </div>
                <div class="mb-3">
                  <label for="email" class="form-label">Email</label>
                  <input type="email" id="email" name="email" class="form-control" value="{{ $user[0]->email }}" readonly>
                </div>
  
                <div class="mb-3">
                  <label for="myTextarea" class="form-label">Adresse</label>
                  <textarea id="myTextarea" name="adresse" class="form-control" rows="3" readonly>{{ $user[0]->client->adresse }}</textarea>
                </div>
              ...
              <div class="mb-3" id="permis_images">
                <label for="adresse" class="form-label">Permis de conduire</label>
                @if($user[0]->client->permi != null)
                <div class="row">
                    <div class="form-group col-md-6">
                        <img id="permi-recto-preview" src="{{ asset($user[0]->client->permi) }}" alt="Permi Recto Preview" style="width: 100%">
                    </div>
                    <div class="form-group col-md-6">
                        <img id="v-permi-verso-preview" src="{{ asset($user[0]->client->v_permi) }}" alt="Permi Verso Preview" style="width: 100%">
                    </div>
                </div>
                @endif
            </div>        
                ...
                <div class="mb-3">
                  ...
                  <input type="submit"  value="Save" id="savebtn" class="form-control bg-primary" style="color: white !important ;  display: none" value="{{ $user[0]->client->phone }}" >
                </div>
